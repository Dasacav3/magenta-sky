<x-jet-action-section>
    <x-slot name="title">
        {{ __('Autenticacion en dos pasos') }}
    </x-slot>

    <x-slot name="description">
        {{ __('Añade seguridad adicional a tu cuenta con la autenticacion en dos pasos.') }}
    </x-slot>
    ...
    <x-slot name="content">
        <h3 class="text-lg font-medium text-gray-900">
            ...
            @else
                {{ __('No tienes activada la autenticación en dos pasos.') }}
            @endif
        </h3>
        ...
        @if ($this->enabled)
            ...
            @if ($showingRecoveryCodes)
                <div class="mt-4 max-w-xl text-sm text-gray-600">
                    <p class="font-semibold">
                        {{ __('Guarda estos codigos en un lugar seguro. Estos pueden ser usados para recuperar el acceso a tu cuenta si la autenticación por dos pasos del dispositivo esta perdida') }}
                    </p>
                </div>
